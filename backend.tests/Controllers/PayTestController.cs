...
//             Assert.Equal(350, pay.ExpectedPay); //2 hours * 175kr/h

//             userController.putUser(uid, 25, "NOK");

//             pay = payController.ExpectedPay(uid, 20231201, 20231230);

//             Assert.Equal(262.5, pay.ExpectedPay); //2 hours * 175kr/h
...
//             Assert.Equal(637.5, pay.ExpectedPay); //2 hours * 250kr/h
...
//             Assert.Equal(637.5, pay.ExpectedPay); //shouldnt include the new shift since out of search range
...
//             Assert.Equal(700, pay.ExpectedPay); //2 hours * 175kr/h should be * 2 since rule
...
//             Assert.Equal(1050, pay.ExpectedPay); //2 hours * 175kr/h should be * 2 since rule
...
//             rule2.Rate = 100;
//             rule2.RateType = "Flat"; //Flat rate of 100 
//             rule2.JobID = job1id;

//             ruleController.PostRule(rule2); //post new rule, calc pay which should be 100extra now

//             pay = payController.ExpectedPay(uid, 20231201, 20231230); //succsessfully ignored non "monday" rule day
//             Assert.Equal(1250, pay.ExpectedPay); //should be 1050 + 100*2 since 2 shifts with 100 flat between 11 and 12
...
//             Assert.Equal(3275, pay.ExpectedPay); //should be 1050 + 100*2 since 2 shifts with 100 flat between 11 and 12
...
//             Assert.Equal(4775, pay.ExpectedPay); //with new rule we should add 
